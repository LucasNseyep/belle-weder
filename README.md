...
The latest work has shifted to the NASA GLOBE Observer dataset and a DenseNet121 + TopBlock architecture trained with Focal Loss and gradual unfreezing. The best result so far — **~50% accuracy on 7-class cloud classification** — comes from a hybrid dataset strategy: extracting 224×224 patches from GLOBE images for the five shared cloud types and using clean GCD benchmark images for the two GCD-exclusive classes (clear sky and mixed). The main remaining bottleneck is dataset size and inter-class ambiguity between visually similar cloud types.
...
In the moment, we couldn't check the weather due to the lack of service in the region. Thankfully, we were fine most of the way, apart from two tire punctures, but other travellers weren't as lucky. We observed mudslides, flooding, mini-buses being pushed up hills, people stranded on the side of the road...
...
- Create a standalone device that can be attached to urban and rural infrastructure (street poles, watchtowers, buoys in the sea, boat masts, etc…) to continuously take measures and make predictions.

## So, what have I wrought so far?

### Early Work — CCSN Dataset & Classical ML

*Chose a starting dataset*
For the computer vision side of this project, we're using the CCSN dataset of cloud images.

The CCSN dataset contains 2543 cloud images divided into 11 categories: **Ac, Sc, Ns, Cu, Ci, Cc, Cb, As, Ct, Cs, St**. **Ci** = cirrus; **Cs** = cirrostratus; **Cc** = cirrocumulus; **Ac** = altocumulus; **As** = altostratus; **Cu** = cumulus; **Cb** = cumulonimbus; **Ns** = nimbostratus; **Sc** = stratocumulus; **St** = stratus; **Ct** = contrail. All images are fixed resolution 256×256 pixels with the JPEG format.

![CCSN Dataset Label Distributions](resources/figures/ccsn_label_distributions.png)

*Why this dataset?*
I chose CCSN because of its completeness. It contains a fair amount of images for all the key clouds we observe in the atmosphere.

![Diagram Containing All Clouds](resources/figures/cloud_types_diagram.png)
<sup><sub>This diagram does not contain contrails. [Contrails](https://weather.metoffice.gov.uk/learn-about/weather/types-of-weather/clouds/unusual-cloud-formations/contrails) are long thin lines of clouds created from water vapour coming out of an aircraft's engines. Height: above 20,000 ft. They're often seen criss-crossing the sky.</sub></sup>


Very importantly, Different types of clouds can give us clues about what is happening in the atmosphere – and what we can expect the weather to do.
...
Following this, I moved on to exploring transfer learning using `vit_b_16` and `convnext`. This is because transfer learning reduces required dataset size, and improves generalisability by reusing pretrained model layers, effectively also reducing computation costs.
...
# Checkpoint 1 — NASA GLOBE Baseline: EDA, Augmentation & Classifier Sweep
...
UPDATE (05/01/2026):
* **Transfer Learning Milestone:** Implemented pre-trained models via PyTorch, achieving a significant accuracy jump from **0.30 to 0.50** using `vit_b_16`.
* **Benchmark Disparity:** Observed that the same model architecture reaches ~0.90 accuracy on the *[102 Category Flower Dataset](https://docs.pytorch.org/vision/0.17/generated/torchvision.datasets.Flowers102.html)* dataset, highlighting the unique difficulty of cloud feature extraction compared to standard objects.
